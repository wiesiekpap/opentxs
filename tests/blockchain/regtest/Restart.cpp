...
TEST_F(Restart_fixture, send_to_client_reboot_confirm_data)
{
    ...
    // Compare balance
    EXPECT_TRUE(
        GetBalance(user_bob_after_reboot) == receiver_balance_after_send);
    //    EXPECT_EQ(user_bob_after_reboot.PaymentCode(), bobs_payment_code);
    EXPECT_TRUE(user_bob_after_reboot.PaymentCode() == bobs_payment_code);

    EXPECT_TRUE(
        GetBalance(user_alice_after_reboot) == sender_balance_after_send);
    std::cerr << "user_alice_after_reboot: " << std::hex
              << GetBalance(user_alice_after_reboot) << std::dec
              << "sender_balance_after_send: " << std::hex
              << sender_balance_after_send << std::dec << "\n";

    //    EXPECT_EQ(user_alice_after_reboot.PaymentCode(), alice_payment_code);
    EXPECT_TRUE(user_alice_after_reboot.PaymentCode() == alice_payment_code);
