    ...
    std::chrono::seconds max_duration) -> void
{
    ...
    while (now < end) {
        ...
        std::cerr << "Waiting for synchronization, balance: " << std::hex
                  << GetBalance(user) << std::dec
                  << ", expected balance: " << std::hex << expected_balance
                  << std::dec << ", sync percentage: " << percentage.str()
                  << "%, sync progress [" << std::to_string(progress.first)
