...
struct PeerListener::Imp {
    ...
    Imp(PeerListener& parent,
        ...
        , lock_{}
        , miner_1_cb_{ot::network::zeromq::ListenCallback::Factory(
              [this](auto&& msg) {
                  cb(std::move(msg), parent_.miner_1_peers_);
              })}
        , ss_cb_{ot::network::zeromq::ListenCallback::Factory(
              [this](auto&& msg) {
        ...
        , m_socket_{miner.Network().ZeroMQ().SubscribeSocket(
              miner_1_cb_,
              "MINER\0"sv)}
        , ss_socket_{syncServer.Network().ZeroMQ().SubscribeSocket(
              ss_cb_,
              "SYNSERV\0"sv)}
        , c1_socket_{client1.Network().ZeroMQ().SubscribeSocket(
              client_1_cb_,
              "CLIENT1\0"sv)}
        , c2_socket_{client2.Network().ZeroMQ().SubscribeSocket(
              client_2_cb_,
              "CLIENT2\0"sv)}
    {
        if (!m_socket_->Start(
...
std::unique_ptr<PeerListener> Regtest_fixture_base::peer_listener_{};
