...
{
    EXPECT_TRUE(
        authority_->hasCapability(identity::NymCapability::SIGN_CHILDCRED));
    EXPECT_TRUE(
        authority_->hasCapability(identity::NymCapability::SIGN_MESSAGE));
}
...
TEST_F(Test_Authority, Serialize_AddedCredentialsFirst_ShouldReturnProperData)
{
    ...
    identity::CredentialIndexModeFlag mode =
        identity::CREDENTIAL_INDEX_MODE_ONLY_IDS;
