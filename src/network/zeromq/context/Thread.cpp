...
{
    if (!thread_.running_ || name.empty()) { return false; }
    ...
    const std::string_view threadName) noexcept -> bool
{
    ...
    if (from_reactor()) {
        ...
        if (thread_name_.empty() && !threadName.empty()) {
            SetThisThreadsName(threadName);
            SetName(threadName);
            thread_name_ = threadName;
        }
    } else {  // TODO get rid of thread_name_
        std::unique_lock<std::mutex> task_lock(task_mtx_);
        tasks_.emplace_back([this, skts = std::move(sockets)]() mutable {
            snc_add(std::move(skts));
        });
        if (thread_name_.empty() && !threadName.empty()) {
            tasks_.emplace_back([this, name = threadName]() mutable {
