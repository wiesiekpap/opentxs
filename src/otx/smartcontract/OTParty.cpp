...
auto OTParty::AddAgent(OTAgent& theAgent) -> bool
{
    ...
    if (!OTScriptable::ValidateName(str_agent_name)) {
        LogError()(OT_METHOD)(__func__)(": Failed validating Agent name.")
            .Flush();
        return false;
    }
...
auto OTParty::AddAccount(OTPartyAccount& thePartyAcct) -> bool
{
    ...
    if (!OTScriptable::ValidateName(str_acct_name)) {
        LogError()(OT_METHOD)(__func__)(": Failed validating Account name.")
            .Flush();
        return false;
    }
    ...
    -> std::int64_t
{
    ...
    if (m_mapPartyAccounts.end() == it)  // If it wasn't already there...
    {
        LogConsole()(OT_METHOD)(__func__)(": Failed -- Account wasn't found: ")(
            str_for_acct_name)(".")
            .Flush();
        return 0;
    }
...
auto OTParty::SetPartyName(const std::string& str_party_name_input) -> bool
{
    if (!OTScriptable::ValidateName(str_party_name_input)) {
        LogError()(OT_METHOD)(__func__)(": Failed: Invalid name was passed in.")
            .Flush();
        return false;
    }
    ...
    -> OTPartyAccount*
{
    ...
    if (OTScriptable::ValidateName(str_acct_name)) {
        ...
        LogError()(OT_METHOD)(__func__)(": Failed: str_acct_name is invalid.")
            .Flush();
    ...
    -> OTPartyAccount*
{
    if (OTScriptable::ValidateName(str_agent_name)) {
        ...
        LogError()(OT_METHOD)(__func__)(": Failed: str_agent_name is invalid.")
            .Flush();
                                      ...
                                      // it was there.
{
    if (OTScriptable::ValidateName(m_str_authorizing_agent)) {
        ...
        if (m_mapAgents.end() != it)  // If we found something...
        {
            ...
            LogError()(OT_METHOD)(__func__)(": Named agent wasn't found "
                                            "on list.")
                .Flush();
    }
                                      ...
                                      // there.
{
    if (OTScriptable::ValidateName(m_str_authorizing_agent)) {
        ...
        if (m_mapAgents.end() != it)  // If we found something...
        {
            ...
            LogError()(OT_METHOD)(__func__)(": Named agent wasn't "
                                            "found on list.")
                .Flush();
    }
    ...
    if (OTScriptable::ValidateName(m_str_authorizing_agent)) {
        ...
        if (m_mapAgents.end() != it)  // If we found something...
        {
            ...
            if (!pAgent->IsAnIndividual())
                ...
                LogError()(OT_METHOD)(__func__)(": Failed loading "
                                                "Nym.")
                    .Flush();
            else {
...
auto OTParty::VerifyOwnershipOfAccount(const Account& theAccount) const -> bool
{
                  ...
                  // solitary Nym (not an entity.)
    {
        ...
        if (!bNymID || (str_nym_id.size() <= 0)) {
            LogError()(OT_METHOD)(__func__)(": Although party is a "
        ...
        LogError()(OT_METHOD)(__func__)(": Error: Entities have not "
                                        "been implemented yet, "
                                        "but somehow this party is an entity.")
            .Flush();
    else
        LogError()(OT_METHOD)(__func__)(": Error: Unknown party "
                                        "type.")
            .Flush();
    ...
    OTString pstrAttachment) -> bool
{
    ...
    if (nullptr == m_pOwnerAgreement) {
        LogError()(OT_METHOD)(__func__)(": Missing pointer to owner agreement.")
            .Flush();
        return false;
    } else if (
        nullptr ==
    ...
    OTString pstrAttachment) -> bool
{
    ...
    if (nullptr == m_pOwnerAgreement) {
        LogError()(OT_METHOD)(__func__)(": Missing pointer to "
                                        "owner agreement.")
            .Flush();
        return false;
    } else if (
        nullptr ==
        (pSmartContract = dynamic_cast<OTSmartContract*>(m_pOwnerAgreement))) {
        LogError()(OT_METHOD)(__func__)(": Can only drop "
    ...
    for (auto& it : m_mapAgents) {
        ...
        if (false == pAgent->DropFinalReceiptToNymbox(
                         *pSmartContract,
                         ...
                         pstrNote,
                         pstrAttachment))
            LogError()(OT_METHOD)(__func__)(": Failed dropping "
    ...
    mapOfAccounts& map_NewlyLoaded) -> bool
{
    ...
    for (auto& it_acct : m_mapPartyAccounts) {
        ...
        if (!strAcctID.Exists()) {
            LogConsole()(OT_METHOD)(__func__)(": Bad: Acct ID is "
                                              "blank for account: ")(
        ...
        if (theAcctIDSet.end() == it_acct_id)  // It's not already there (good).
        {
            ...
            LogConsole()(OT_METHOD)(__func__)(": Failure: Found a "
                                              "duplicate Acct ID (")(
        ...
        if (bHadToLoadtheAcctMyself == true) {
            if ((account = pPartyAcct->LoadAccount())) {
                LogConsole()(OT_METHOD)(__func__)(": Failed loading "
                                                  "Account with name: ")(
    ...
    const PasswordPrompt& reason)
{
    for (auto& it : m_mapPartyAccounts) {
        ...
        if (nullptr == pAgent) {
            LogError()(OT_METHOD)(__func__)(": Couldn't find agent (")(
    ...
    otx::context::Server& context) const
{
    if (!(GetAuthorizingAgentName().compare(theAgent.GetName().Get()) == 0)) {
        LogError()(OT_METHOD)(__func__)(": Error: Agent name doesn't match: ")(
...
auto OTParty::ReserveTransNumsForConfirm(otx::context::Server& context) -> bool
{
    ...
    if (nullptr == pMainAgent) {
        LogConsole()(OT_METHOD)(__func__)(": Failure: Authorizing "
                                          "agent (")(GetPartyName())(
...
auto OTParty::Compare(const OTParty& rhs) const -> bool
{
    ...
    if ((GetPartyID().size() > 0) && (rhs.GetPartyID().size() > 0) &&
        ...
        LogConsole()(OT_METHOD)(__func__)(": Party IDs don't match for party ")(
    ...
    for (const auto& it : m_mapPartyAccounts) {
        ...
        if (nullptr == p2) {
            LogConsole()(OT_METHOD)(__func__)(": Unable to find Account ")(
        ...
        if (!pAcct->Compare(*p2)) {
            LogConsole()(OT_METHOD)(__func__)(": Accounts (")(
...
auto OTParty::CopyAcctsToConfirmingParty(OTParty& theParty) const -> bool
{
    ...
    for (const auto& it : m_mapPartyAccounts) {
        ...
        if (false == theParty.AddAccount(
            ...
            LogConsole()(OT_METHOD)(__func__)(": Unable to add Account ")(
