...
// Only need to look inside a message to compensate for duplicate scheduled
    ...
    std::chrono::time_point<std::chrono::system_clock> t_at) -> bool
{
    ...
    // Compensating for business logic errors here
    const auto& body = in.Body();
    if (body.size() == 0) { return false; }
    const auto& mtype = body.at(0);
    std::unique_lock<std::mutex> lck(mtx_queue_state);
    if (!scheduler_queue_.empty()) {
        if (mtype == scheduler_queue_.top().m_.Body().at(0)) { return true; }
    }
    // end of duplicate reduction
...
bool Reactor::stop()
{
    ...
    notify();
    if (can_join) {
        // TODO optimize the diagnostic for production builds
        auto who = ThreadMonitor::get_name();
        auto whom = ThreadMonitor::get_name(thread_->native_handle());
        tdiag(who + " about to join ", whom);
        thread_->join();
        tdiag(who + " joined ", whom);
        std::unique_lock<std::mutex> lck(mtx_queue_state);
        thread_.reset();
    }
    return was_active;
