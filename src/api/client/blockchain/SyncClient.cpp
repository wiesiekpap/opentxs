...
struct SyncClient::Imp {
    ...
    auto Init() noexcept -> void { thread_ = std::thread{&Imp::thread, this}; }

    Imp(const api::Core& api, const Blockchain& parent) noexcept
        : api_(api)
        , parent_(parent)
        ...
        , external_router_([&] {
            auto out =
                Socket{::zmq_socket(api_.ZeroMQ(), ZMQ_ROUTER), ::zmq_close};
...
private:
    ...
    auto first() noexcept -> void
    {
        for (const auto& ep : parent_.GetSyncServers()) { process_server(ep); }
    }
            ...
            switch (type) {
                case Type::sync_ack: {
                    ...
                    for (const auto& state : ack.State()) {
                        ...
                        auto msg = api_.ZeroMQ().Message(client);
                        msg->AddFrame();
                ...
                case Type::sync_reply: {
                    ...
                    auto msg = api_.ZeroMQ().Message(identity);
                    msg->AddFrame();
    ...
    auto process_internal(const Message& msg) noexcept -> void
    {
        ...
        switch (type) {
            ...
            case Task::Request: {
                ...
                auto msg = api_.ZeroMQ().Message(provider);
                msg->AddFrame();
    ...
    auto state_machine() noexcept -> void
    {
        ...
        for (auto& [endpoint, server] : servers_) {
            ...
            if (server.needs_query()) {
                auto msg = api_.ZeroMQ().Message(endpoint);
                msg->AddFrame();
...
auto SyncClient::Init() noexcept -> void { return imp_.Init(); }
