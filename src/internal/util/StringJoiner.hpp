...
size_t string_size(String&& s)
{
    ...
    using string_t = typename std::conditional_t<
        std::is_array_v<noref_t>,
        noref_t,
        typename std::remove_cv_t<noref_t>>;
...
struct concatenate_impl<String, Rest...> {
    ...
    {
        result += s;
        concatenate_impl<Rest...>::concatenate(
...
{
    std::string result;
    result.reserve(details::concatenate_impl<Strings...>::size(
        std::forward<Strings>(strings)...));
    details::concatenate_impl<Strings...>::concatenate(
